...
func googleNew(stdout *common.Stdout) *vendors.Google {
	...
	google := vendors.NewGoogle(googleOptions, stdout)
	if google == nil {
		stdout.Panic("No google")
	}
	return google
}
...
func NewGoogleCommand() *cobra.Command {
	...
	calendarEventsCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			googleOptions.CalendarOptions = &googleCalendarOptions
			bytes, err := googleNew(stdout).CalendarGetEvents()
			if err != nil {
				stdout.Error(err)
				return
			}
